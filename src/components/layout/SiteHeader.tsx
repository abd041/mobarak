...
export function SiteHeader({ locale }: { locale: Locale }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#eceef1] bg-white">
      <div className="relative mx-auto flex h-16 max-w-page items-center gap-2 px-4 sm:px-5 md:px-8 min-[1180px]:h-[4.5rem] lg:px-9.25">
        <Link
          href="/"
        ...
        >
          <BrandLogo height={40} className="min-[1180px]:hidden" priority />
          <BrandLogo height={56} className="hidden min-[1180px]:block" priority />
        </Link>

        <nav className="absolute left-1/2 hidden min-w-0 -translate-x-1/2 items-center justify-center gap-6 min-[1180px]:flex xl:gap-8">
          {NAV.map((item) => {
        ...
        <div
          className={cn(
            "ms-auto flex shrink-0 items-center gap-1.5 sm:gap-2.5 min-[1180px]:gap-13.75",
        ...
        >
          ...
          <a
            href={`tel:${tMeta("phone").replace(/\s/g, "")}`}
            className="inline-flex h-10 w-10 items-center justify-center rounded-full text-[#0A1B3D] transition hover:bg-[#F7F8FA] min-[1180px]:hidden"
      ...
      {open && (
        <div
          id="mobile-nav"
          className="max-h-[calc(100dvh-4rem)] overflow-y-auto border-t border-[#eceef1] bg-white min-[1180px]:hidden"
        >
          <nav className="mx-auto flex max-w-page flex-col gap-1 px-4 py-3" aria-label={tCommon("menu")}>
            ...
            <div className="mt-3 border-t border-line pt-3">
              <p className="mb-2 px-4 text-[12px] font-semibold tracking-wide text-muted uppercase">
                {tLang("label")}
              </p>
              <LanguageSwitcher
                current={current as Locale}
            ...
            <a
              href={`tel:${tMeta("phone").replace(/\s/g, "")}`}
              className="dir-ltr-keep mt-3 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#0A1B3D] px-4 py-3.5 text-[15px] font-semibold text-white"
            >
  ...
  return (
    <div ref={rootRef} className={cn("relative shrink-0", className)}>
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cn(
          "inline-flex h-11 items-center gap-1.5 rounded-full border border-[#E6E8EC] bg-[#F7F8FA] px-3 text-[12px] font-semibold tracking-[0.04em] text-[#0A1B3D] transition sm:h-9",
      ...
      >
        <Globe className="h-3.5 w-3.5 text-[#1E5A9C]" strokeWidth={2} />
        <span>{current.toUpperCase()}</span>
        <ChevronDown
          className={cn(
            "h-3.5 w-3.5 text-[#6B7C8F] transition-transform duration-200",
            open && "rotate-180",
          )}
          aria-hidden
        />
