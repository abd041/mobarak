"""Crop 6 cream circular Umrah ritual icons from ChatGPT sheet."""
from pathlib import Path

import numpy as np
from PIL import Image

SRC = Path(r"D:\mobarak\ChatGPT Image Sep 14, 2026, 06_01_03 PM.png")
...
def main() -> None:
    ...
    if len(runs) != 6:
        ...
        chunk = (x1 - x0 + 1) / 6
        runs = [(int(x0 + i * chunk), int(x0 + (i + 1) * chunk) - 1) for i in range(6)]
        print("fallback", runs)

    OUT.mkdir(parents=True, exist_ok=True)
    for out_i, (x0, x1) in enumerate(runs, 1):
        col = mask[:, x0 : x1 + 1]
